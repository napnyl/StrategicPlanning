...
{
    public partial class MatrixLevel3 : System.Windows.Forms.Form
    {
        public static DataTable dtLevel3 = new DataTable();
        ...
        string headManagerSt = "DESARROLLO DE ESTRATEGIAS NIVEL 3";
            ...
            if (!fromDb)
            {
                if (dtLevel3.Rows.Count == 0)
                {
                    ...
                    if (dtLevel3.Rows.Count > 0)
                        ...
                        errorMessage = "Debe ingresar al menos una estrategia de Nivel 3 en la matriz básica";
                }
                else
                {
                    if (XtraMessageBox.Show("¿Desea borrar la anterior matriz de nivel 3 y volver a generar una nueva?", "Confirmación", MessageBoxButtons.YesNo) == System.Windows.Forms.DialogResult.Yes)
                    {
                        dtLevel3 = new DataTable();
                        CreateManagerStMatrix();
                        if (dtLevel3.Rows.Count > 0)
                            stShown = true;
                        else
        ...
        {
            dtLevel3 = GroupStrategies(MatrixStrategies.dtStrategies);
            gridControl1.DataSource = dtLevel3;
        ...
        {
            if (e.CloseReason == CloseReason.UserClosing && !Main.closeFromMain && ((BscLevel3.dtBscLevel3 != null && BscLevel3.dtBscLevel3.Rows.Count > 0) || (BscPubLevel3.dtBscLevel3 != null && BscPubLevel3.dtBscLevel3.Rows.Count > 0)))
            {
                XtraMessageBox.Show("Primero se debe cerrar la matriz BSC de Nivel 3 Generada!");
                e.Cancel = true;
            }
            else
            {
        ...
        {
            ((DevExpress.XtraBars.Ribbon.RibbonControl)(this.Parent.TopLevelControl.Controls["ribbonControl1"])).Items["btnManagerLevel3"].Enabled = true;
            dtLevel3 = new DataTable();
            Main.projectSaved = false;
        }
        ...
        {
            DataTable dtManagerStTemp = dtLevel3.Copy();
            dtLevel3 = new DataTable();
            dtLevel3 = GroupStrategies(MatrixStrategies.dtStrategies);

            foreach (DataRow row in dtLevel3.Rows)
            {
                if (dtManagerStTemp.Rows.Count == dtLevel3.Rows.Count)
                {
                    if (dtManagerStTemp.Rows[dtLevel3.Rows.IndexOf(row)][1] != DBNull.Value && dtManagerStTemp.Rows[dtLevel3.Rows.IndexOf(row)][1] != null)
                    {
                        row[headManagerSt] = dtManagerStTemp.Rows[dtLevel3.Rows.IndexOf(row)][1].ToString();
                    }
                    if (dtManagerStTemp.Rows[dtLevel3.Rows.IndexOf(row)][2] != DBNull.Value && dtManagerStTemp.Rows[dtLevel3.Rows.IndexOf(row)][2] != null)
                    {
                        row[headGroup] = dtManagerStTemp.Rows[dtLevel3.Rows.IndexOf(row)][2].ToString();
                    }
                    if (dtManagerStTemp.Rows[dtLevel3.Rows.IndexOf(row)][3] != DBNull.Value && dtManagerStTemp.Rows[dtLevel3.Rows.IndexOf(row)][3] != null)
                    {
                        row[headPerspective] = dtManagerStTemp.Rows[dtLevel3.Rows.IndexOf(row)][3].ToString();
                    }
                }
                else if (dtManagerStTemp.Rows.Count > dtLevel3.Rows.Count)
                {
        ...
        private void btnExportExcel_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            saveDialog.FileName = "Estrategias_Nivel3_" + DateTime.Now.ToString("yyyyMMdd");
