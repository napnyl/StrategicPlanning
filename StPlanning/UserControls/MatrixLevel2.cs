...
{
    public partial class MatrixLevel2 : System.Windows.Forms.Form
    {
        public static DataTable dtLevel2 = new DataTable();
        ...
        string headManagerSt = "DESARROLLO DE ESTRATEGIAS NIVEL 2";
            ...
            if (!fromDb)
            {
                if (dtLevel2.Rows.Count == 0)
                {
                    ...
                    if (dtLevel2.Rows.Count > 0)
                        ...
                        errorMessage = "Debe ingresar al menos una estrategia de Nivel 2 en la matriz básica";
                }
                else
                {
                    if (XtraMessageBox.Show("¿Desea borrar la anterior matriz de nivel 2 y volver a generar una nueva?", "Confirmación", MessageBoxButtons.YesNo) == System.Windows.Forms.DialogResult.Yes)
                    {
                        dtLevel2 = new DataTable();
                        CreateManagerStMatrix();
                        if (dtLevel2.Rows.Count > 0)
                            stShown = true;
                        else
        ...
        {
            dtLevel2 = GroupStrategies(MatrixStrategies.dtStrategies);
            gridControl1.DataSource = dtLevel2;
        ...
        private DataTable GroupStrategies(DataTable dt)
        {
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                if (row[3].ToString().Contains("2"))
                {
                    int lookUpResult = -1;
                    if (row[2] != DBNull.Value && row[2] != null && row[2].ToString() != string.Empty && dtManager.Rows.Count > 0)
                        lookUpResult = LookUpColor(dtManager, Convert.ToInt32(row[2]), dt.Rows.IndexOf(row));

                    if (lookUpResult == -1 || row[2] == null || row[2].ToString() == string.Empty)
                    {
        ...
        {
            if (e.CloseReason == CloseReason.UserClosing && !Main.closeFromMain && ((BscLevel2.dtBscLevel2 != null && BscLevel2.dtBscLevel2.Rows.Count > 0) || (BscPubLevel2.dtBscLevel2 != null && BscPubLevel2.dtBscLevel2.Rows.Count > 0)))
            {
                XtraMessageBox.Show("Primero se debe cerrar la matriz BSC de Nivel 2 Generada!");
                e.Cancel = true;
            }
            else
            {
        ...
        {
            ((DevExpress.XtraBars.Ribbon.RibbonControl)(this.Parent.TopLevelControl.Controls["ribbonControl1"])).Items["btnManagerLevel2"].Enabled = true;
            dtLevel2 = new DataTable();
            Main.projectSaved = false;
        }
        ...
        {
            DataTable dtManagerStTemp = dtLevel2.Copy();
            dtLevel2 = new DataTable();
            dtLevel2 = GroupStrategies(MatrixStrategies.dtStrategies);

            foreach (DataRow row in dtLevel2.Rows)
            {
                if (dtManagerStTemp.Rows.Count == dtLevel2.Rows.Count)
                {
                    if (dtManagerStTemp.Rows[dtLevel2.Rows.IndexOf(row)][1] != DBNull.Value && dtManagerStTemp.Rows[dtLevel2.Rows.IndexOf(row)][1] != null)
                    {
                        row[headManagerSt] = dtManagerStTemp.Rows[dtLevel2.Rows.IndexOf(row)][1].ToString();
                    }
                    if (dtManagerStTemp.Rows[dtLevel2.Rows.IndexOf(row)][2] != DBNull.Value && dtManagerStTemp.Rows[dtLevel2.Rows.IndexOf(row)][2] != null)
                    {
                        row[headGroup] = dtManagerStTemp.Rows[dtLevel2.Rows.IndexOf(row)][2].ToString();
                    }
                    if (dtManagerStTemp.Rows[dtLevel2.Rows.IndexOf(row)][3] != DBNull.Value && dtManagerStTemp.Rows[dtLevel2.Rows.IndexOf(row)][3] != null)
                    {
                        row[headPerspective] = dtManagerStTemp.Rows[dtLevel2.Rows.IndexOf(row)][3].ToString();
                    }
                }
                else if (dtManagerStTemp.Rows.Count > dtLevel2.Rows.Count)
                {
        ...
        private void btnExportExcel_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            saveDialog.FileName = "Estrategias_Nivel2_" + DateTime.Now.ToString("yyyyMMdd");
